...
export default function MatrixApp() {
  const [activeTab, setActiveTab] = useState<'linear' | 'multiply' | 'advanced'>(() => {
    if (typeof window !== 'undefined') {
      const tab = new URLSearchParams(window.location.search).get('tab');
      if (tab === 'linear' || tab === 'multiply' || tab === 'advanced') return tab;
    }
    return 'linear';
  });
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-950 text-gray-100 font-sans selection:bg-blue-500/30">
      ...
      <aside className="w-full md:w-64 bg-gray-900 shadow-xl border-b md:border-r border-gray-800 flex flex-col shrink-0 md:h-screen md:sticky md:top-0 z-40">
        ...
        <div className={`${isMobileMenuOpen ? 'flex' : 'hidden'} md:flex flex-col p-4 gap-3 flex-1 overflow-y-auto scrollbar-hide mt-4`}>
          <Link href="/matrix" className="flex items-center gap-3 px-4 py-3 rounded-xl font-bold bg-blue-600 text-white shadow-lg shadow-blue-900/20 transition-all">
            Matrix
          </Link>
      ...
      <main className="flex-1 flex flex-col min-w-0 md:h-screen overflow-y-auto">
        ...
        <div key={activeTab} className="p-4 md:p-10 flex flex-col items-center animate-in fade-in slide-in-from-right-8 duration-500 ease-out w-full">
          {activeTab === 'linear' && (
            <div className="w-full max-w-6xl">
              <div className="bg-gray-800 px-5 md:px-6 py-5 rounded-2xl shadow-sm border border-gray-700 w-full mb-8 md:mb-10 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-5 w-full sm:w-auto">
                  ...
                  <div className="flex flex-row items-center gap-3 w-full sm:w-auto">
                    <div className="flex flex-1 sm:flex-none items-center justify-between sm:justify-start gap-3 bg-gray-900 px-3 py-2 rounded-xl border border-gray-700 focus-within:border-gray-500 transition-colors">
                ...
                <button type="button" onClick={addMatrix} className="px-5 py-3 sm:py-2.5 text-sm md:text-base bg-gray-700 text-white font-bold rounded-xl hover:bg-gray-600 active:scale-95 transition-all shadow-sm border border-gray-600 w-full sm:w-auto tracking-wide">
                  + เพิ่มเมทริกซ์
                </button>
              </div>
              ...
              <div className="flex flex-col md:flex-row flex-wrap gap-8 items-center justify-center w-full pb-10">
                    ...
                    <Fragment key={mat.id}>
                      {i > 0 && (
                        <div className="flex justify-center items-center shrink-0">
                          <button type="button" onClick={() => updateOperator(mat.id, mat.operator === '+' ? '-' : '+')} className="w-14 h-14 flex items-center justify-center rounded-full bg-gray-800 border-2 border-gray-600 text-3xl font-bold text-gray-300 hover:border-gray-400 hover:shadow-lg active:scale-90 transition-all outline-none pb-1">
                            {mat.operator}
                          </button>
                        </div>
                      )}
                      <div className="flex flex-col bg-gray-800 p-5 md:p-6 rounded-2xl shadow-sm border border-gray-700 w-full max-w-[18rem] md:w-auto mx-auto md:mx-0 shrink-0 hover:border-gray-600 transition-colors">
                        <div className="flex justify-between items-center w-full mb-5 border-b border-gray-700 pb-4">
                          <div className="flex items-center gap-3">
                            <input type="text" placeholder="k" className="w-12 px-2 py-1.5 border border-gray-600 rounded-lg text-center outline-none font-bold text-white bg-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm transition-all" value={mat.scalar} onChange={(e) => updateScalar(mat.id, e.target.value)} />
                            <span className="font-extrabold text-gray-200 tracking-wide">Matrix {matrixNames[i]}</span>
                          </div>
                          <div className="flex gap-2">
                            <button type="button" onClick={() => clearMatrix(mat.id)} className="text-xs font-bold text-gray-400 hover:text-white px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 active:scale-95 transition-all">ล้าง</button>
                            {matrices.length > 1 && (
                              <button type="button" onClick={() => removeMatrix(mat.id)} className="text-xs font-bold text-red-400 hover:text-red-200 px-3 py-1.5 rounded-lg bg-red-900/40 hover:bg-red-800/60 active:scale-95 transition-all">ลบ</button>
                            )}
                          </div>
                        </div>
                        <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(linearCols)}`}>
                          {Array.from({ length: linearRows * linearCols }).map((_, i) => {
                            ...
                            return (
                              <label key={`mat-${mat.id}-${stateIdx}`} className={`relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 border-2 rounded-xl cursor-text transition-all ${isActiveScalar ? 'border-blue-500/50 bg-blue-900/20' : 'border-gray-600 bg-gray-900 focus-within:border-gray-400'}`}>
                                <input type="number" className="w-full h-full text-center outline-none bg-transparent text-xl font-bold text-white z-10" value={mat.data[stateIdx]} onChange={(e) => updateMatrixData(mat.id, stateIdx, e.target.value)} placeholder="0" />
                ...
                <div className="flex justify-center items-center shrink-0">
                  <span className="text-4xl font-black text-gray-600 md:px-2">=</span>
                </div>
                <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 text-white w-full max-w-[18rem] md:w-auto mx-auto md:mx-0 shrink-0">
                  <p className="text-center mb-5 font-extrabold tracking-widest text-lg border-b border-gray-700 pb-4 text-blue-400">RESULT</p>
                  <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(linearCols)}`}>
                    {Array.from({ length: linearRows * linearCols }).map((_, i) => {
                      const stateIdx = Math.floor(i / linearCols) * 4 + (i % linearCols);
                      return (
                        <div key={`res-${stateIdx}`} className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-gray-900 border-2 border-gray-800 rounded-xl text-xl font-bold shadow-inner overflow-hidden text-gray-100">
          ...
          {activeTab === 'multiply' && (
            <div className="w-full max-w-6xl">
              <div className="bg-gray-800 px-5 md:px-6 py-5 rounded-2xl shadow-sm border border-gray-700 w-full mb-8 md:mb-10 flex flex-col lg:flex-row gap-5 items-start lg:items-center justify-between">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-5 w-full lg:w-auto overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
                  ...
                  <div className="flex flex-row items-center gap-3 w-full sm:w-auto">
                    <div className="flex flex-1 sm:flex-none items-center justify-between gap-2 bg-gray-900 px-3 py-2 rounded-xl border border-gray-700 focus-within:border-gray-500 transition-colors shrink-0">
              ...
              <div className="flex flex-col md:flex-row flex-wrap gap-8 items-center justify-center w-full pb-10">
                <div className="flex flex-col bg-gray-800 p-5 md:p-6 rounded-2xl shadow-sm border border-gray-700 w-full max-w-[18rem] md:w-auto mx-auto md:mx-0 shrink-0 hover:border-gray-600 transition-colors">
                  <div className="flex justify-between items-center w-full mb-5 border-b border-gray-700 pb-4">
                    <div className="flex items-center gap-3">
                      <input type="text" placeholder="k" className="w-12 px-2 py-1.5 border border-gray-600 rounded-lg text-center outline-none font-bold text-white bg-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm transition-all" value={scalarMultA} onChange={(e) => setScalarMultA(e.target.value)} />
                  ...
                  <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(colA)}`}>
                    {Array.from({ length: rowA * colA }).map((_, i) => {
                      ...
                      return (
                        <label key={`A-${stateIdx}`} className={`relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 border-2 rounded-xl cursor-text transition-all ${isActive ? 'border-blue-500/50 bg-blue-900/20' : 'border-gray-600 bg-gray-900 focus-within:border-gray-400'}`}>
                          <input type="number" className="w-full h-full text-center outline-none bg-transparent text-xl font-bold text-white z-10" value={matrixMultA[stateIdx]} onChange={(e) => updateMultMatrix('A', stateIdx, e.target.value)} placeholder="0" />
                ...
                <div className="flex justify-center items-center shrink-0">
                  <div className="w-14 h-14 flex items-center justify-center rounded-full bg-gray-800 border-2 border-gray-600 text-4xl font-bold text-gray-400 shadow-sm pb-1">×</div>
                </div>

                <div className="flex flex-col bg-gray-800 p-5 md:p-6 rounded-2xl shadow-sm border border-gray-700 w-full max-w-[18rem] md:w-auto mx-auto md:mx-0 shrink-0 hover:border-gray-600 transition-colors">
                  <div className="flex justify-between items-center w-full mb-5 border-b border-gray-700 pb-4">
                     <div className="flex items-center gap-3">
                      <input type="text" placeholder="k" className="w-12 px-2 py-1.5 border border-gray-600 rounded-lg text-center outline-none font-bold text-white bg-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm transition-all" value={scalarMultB} onChange={(e) => setScalarMultB(e.target.value)} />
                  ...
                  <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(colB)}`}>
                    {Array.from({ length: colA * colB }).map((_, i) => {
                      ...
                      return (
                        <label key={`B-${stateIdx}`} className={`relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 border-2 rounded-xl cursor-text transition-all ${isActive ? 'border-blue-500/50 bg-blue-900/20' : 'border-gray-600 bg-gray-900 focus-within:border-gray-400'}`}>
                          <input type="number" className="w-full h-full text-center outline-none bg-transparent text-xl font-bold text-white z-10" value={matrixMultB[stateIdx]} onChange={(e) => updateMultMatrix('B', stateIdx, e.target.value)} placeholder="0" />
                ...
                <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 text-white w-full max-w-[18rem] md:w-auto mx-auto md:mx-0 flex flex-col items-center shrink-0">
                  <div className="w-full flex justify-between items-center mb-5 border-b border-gray-800 pb-4 gap-6">
                    <span className="font-extrabold tracking-widest text-lg text-blue-400">RESULT</span>
                    <button onClick={useResultAsA} className="text-sm bg-blue-600 hover:bg-blue-500 text-white font-bold px-4 py-2 rounded-lg transition active:scale-95 shadow-sm border border-blue-500 tracking-wide">ใช้เป็นตัวตั้ง</button>
                  </div>
                  <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(colB)}`}>
                    {Array.from({ length: rowA * colB }).map((_, i) => {
                      ...
                      for (let k = 0; k < colA; k++) resultSum += (getNum(matrixMultA[r * 4 + k]) * kA) * (getNum(matrixMultB[k * 4 + c]) * kB);
                      return (
                        <div key={`res-${i}`} className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-gray-900 border-2 border-gray-800 rounded-xl text-xl font-bold shadow-inner overflow-hidden text-gray-100">{resultSum}</div>
                      );
                    })}
                  </div>
                </div>
          ...
          {activeTab === 'advanced' && (
            <div className="w-full max-w-6xl flex flex-col items-center">
              <div className="bg-gray-800 p-5 md:p-6 rounded-2xl shadow-sm border border-gray-700 w-full max-w-[22rem] mb-10 flex flex-col mx-auto hover:border-gray-600 transition-colors">
                <div className="flex justify-between items-center w-full mb-5 border-b border-gray-700 pb-4">
                  <span className="font-extrabold text-gray-100 text-xl tracking-wide">Matrix A</span>
                  <button type="button" onClick={clearAdvMatrix} className="text-xs font-bold text-gray-400 hover:text-white px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 active:scale-95 transition-all">ล้างค่า</button>
                </div>
                ...
                <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(advCols)}`}>
                  {Array.from({ length: advRows * advCols }).map((_, i) => {
                    ...
                    return (
                      <label key={`adv-${stateIdx}`} className="relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 border-2 border-gray-600 bg-gray-900 focus-within:border-gray-400 rounded-xl cursor-text transition-all shadow-inner">
                        <input type="number" className="w-full h-full text-center outline-none bg-transparent text-2xl font-bold text-white" value={matrixAdv[stateIdx]} onChange={(e) => updateAdvMatrix(stateIdx, e.target.value)} placeholder="0" />
                      </label>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 text-white flex flex-col items-center">
                  <p className="text-center font-extrabold tracking-wider text-lg border-b border-gray-700 pb-4 w-full mb-5 text-gray-300">Transpose (Aᵀ)</p>
                  <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(advRows)}`}>
                    {Array.from({ length: advCols * advRows }).map((_, i) => {
                      ...
                      const c = i % advRows;
                      return (
                        <div key={`trans-${i}`} className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-gray-900 border-2 border-gray-800 rounded-xl text-xl font-bold shadow-inner overflow-hidden text-gray-100">
                          {adv2D[c][r]}
                        </div>
                      );
                    })}
                ...
                <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 text-white flex flex-col items-center justify-start">
                  <p className="text-center font-extrabold tracking-wider text-lg border-b border-gray-700 pb-4 w-full mb-5 text-gray-300">Determinant det(A)</p>
                ...
                <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 text-white flex flex-col items-center">
                  <p className="text-center font-extrabold tracking-wider text-lg border-b border-gray-700 pb-4 w-full mb-5 text-gray-300">Inverse (A⁻¹)</p>
                  {!isSquare ? (
                     <div className="flex-1 flex items-center justify-center w-full min-h-24">
                       <span className="text-sm font-bold text-red-400 bg-red-900/30 border border-red-500/30 px-5 py-2.5 rounded-xl text-center tracking-wide">
                         ต้องเป็นตารางจัตุรัสเท่านั้น
                    ...
                    <div className={`grid gap-3 mx-auto w-fit ${getLinearGridColsClass(advCols)}`}>
                      {advInv?.map((row, r) => row.map((val, c) => (
                        <div key={`inv-${r}-${c}`} className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center bg-gray-900 border-2 border-gray-800 rounded-xl text-sm md:text-base font-bold text-blue-400 shadow-inner overflow-hidden">
                          {formatRes(val)}
                        </div>
                      )))}
                    </div>
